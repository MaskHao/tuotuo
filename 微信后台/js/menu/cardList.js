...
var CardList = {
    ...
    Temp: {
        ...
        Card: '<div class="card">{{#list}}{{#single}}<div data-id="{{mediaId}}" class="card_list btnSelectCard"><a class="cl_cont"><p class="cl_hd">{{first.title}}</p><div class="cl_hdPic" style="background-image: url({{first.thumbUrl}})"></div><p class="cl_des">{{first.description}}</p><hr><p class="cl_ft">阅读全文<span>&gt;</span></p></a>{{#select}}<div class="mask" style="display: block"><i class="icon_card_selected"></i></div>{{/select}}</div>{{/single}}{{^single}}<div data-id="{{mediaId}}" class="card_list btnSelectCard"><div class="cl_cont2"><a class="cl_top"><div class="cltPic" style="background-image:url({{first.thumbUrl}})"></div><p class="cltWord">{{first.title}}</p></a>{{#itemList}}<div class="cl_ct"><a class="tow"><p><span>{{title}}</span></p><div class="pic" style="background-image:url({{thumbUrl}})"></div></a></div>{{/itemList}}</div>{{#select}}<div class="mask" style="display: block"><i class="icon_card_selected"></i></div>{{/select}}</div>{{/single}}{{/list}}</div>'
    },
