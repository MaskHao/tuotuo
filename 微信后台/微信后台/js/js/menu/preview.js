...
var Preview = {
    ...
    Temp: {
        ...
        Card: '{{#single}}<div data-id="{{mediaId}}" class="card_list"><a class="cl_cont" href="{{first.url}}" target="_blank"><p class="cl_hd">{{first.title}}</p><div class="cl_hdPic" style="background-image: url({{first.thumbLink}})"></div><p class="cl_des">{{first.description}}</p><hr><p class="cl_ft">阅读全文<span>&gt;</span></p></a></div>{{/single}}{{^single}}<div data-id="{{mediaId}}" class="card_list"><div class="cl_cont2"><a class="cl_top" href="{{first.url}}" target="_blank"><div class="cltPic" style="background-image:url({{first.thumbLink}})"></div><p class="cltWord">{{first.title}}</p></a>{{#itemList}}<div class="cl_ct"><a class="tow" href="{{url}}" target="_blank"><p><span>{{title}}</span></p><div class="pic" style="background-image:url({{thumbLink}})"></div></a></div>{{/itemList}}</div></div>{{/single}}'
    },
